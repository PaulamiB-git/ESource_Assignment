...
# MAGIC This notebook is called by **ingest_raw_tables** notebook for **utility2**.
...
# MAGIC - **Destination** - Raw Utility2 Tables  (can mention ADLS/S3 location)
...
#ingest network data
df_network = spark.sql('Select * from workspace.default.utility_2_circuits')
...
table_name = "workspace.utility2.raw_utility2_circuits"   #needs to be parameterised based on ADLS location in actual
mode = "overwrite"
...
df_installed = spark.sql('Select * from workspace.default.utility_2_install_der')
df_installed = df_installed.withColumn("year_of_data",lit(year)) \
                           .withColumn("month_of_data",lit(month))\
                             .withColumn("processed_timestamp",lit(processed_timestamp)) 
table_name = "workspace.utility2.raw_utility2_install_der"   #needs to be parameterised based on ADLS location in actual
mode = "overwrite"
partition_cols = ["year_of_data","month_of_data"]
...
#ingest planned data
df_planned = spark.sql('Select * from workspace.default.utility_2_planned_der')
df_planned = df_planned.withColumn("year_of_data",lit(year)) \
                           .withColumn("month_of_data",lit(month))\
                             .withColumn("processed_timestamp",lit(processed_timestamp)) 
table_name = "workspace.utility2.raw_utility2_planned_der"   #needs to be parameterised based on ADLS location in actual
mode = "overwrite"
partition_cols = ["year_of_data","month_of_data"]
...
#%sql
#select count(*) from workspace.utility2.raw_utility2_circuits
#select count(*) from workspace.utility2.raw_utility2_install_der
#select count(*) from workspace.utility2.raw_utility2_planned_der
